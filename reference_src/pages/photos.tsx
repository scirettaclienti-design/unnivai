...
export default function PhotosPage() {
  ...
  const sharePhoto = (platform: string, photo: PhotoData) => {
    ...
    <div className="min-h-screen bg-gradient-to-b from-ochre-100 to-ochre-200 font-quicksand">
      ...
      <main className="max-w-md mx-auto px-4 py-8 pb-24">
        ...
        >
          ...
          <h1 className="text-2xl font-bold text-gray-800 mb-2">Foto dei Tour</h1>
          <p className="text-gray-600">Condividi i tuoi ricordi di viaggio</p>
        </motion.div>

        {/* Action Bar */}
        <motion.div
          className="flex items-center justify-between mb-6 bg-white/70 backdrop-blur-sm rounded-2xl p-4 shadow-lg"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <motion.button
            ...
            className="bg-gradient-to-r from-terracotta-400 to-terracotta-500 text-white p-3 rounded-xl shadow-lg hover:shadow-xl transition-all flex items-center space-x-2"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          >
            <Plus className="w-5 h-5" />
            <span className="font-medium">Carica</span>
          </motion.button>
        </motion.div>
        ...
        <div className={`${viewMode === 'grid' ? 'grid grid-cols-2 gap-4' : 'space-y-6'}`}>
          {filteredPhotos.map((photo, index) => (
            <motion.div
              key={photo.id}
              className={`bg-white/70 backdrop-blur-sm rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all ${
                viewMode === 'feed' ? 'p-4' : ''
              }`}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              ...
              >
                <img
                  ...
                  className={`w-full object-cover ${
                    viewMode === 'grid' ? 'h-48 rounded-2xl' : 'h-64 rounded-xl'
                  }`}
                />
                ...
                <div className="absolute top-3 right-3 flex space-x-2">
                  {photo.tags.slice(0, 2).map(tag => (
                    <span
                      key={tag}
                      className="bg-black/50 text-white text-xs px-2 py-1 rounded-full"
                    >
                      #{tag}
                    </span>
                  ))}
                ...
                {viewMode === 'grid' && (
                  <div className="absolute bottom-3 left-3 right-3">
                    <h4 className="text-white font-bold text-sm mb-1">{photo.tourTitle}</h4>
