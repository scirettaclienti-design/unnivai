...
export default function QuickPathPage() {
  ...
  const handleMainSelection = (optionId: string) => {
    setSelectedOption(optionId);
    setCurrentStep(2);
  };

  const handleSubSelection = (subOption: any) => {
    setSelectedSubOption(subOption);
    setCurrentStep(3);
  };

  const handleTimeSelection = (timeOption: any) => {
    setSelectedTime(timeOption);
    setCurrentStep(4);
  };

  const handleDurationSelection = (durationOption: any) => {
    setSelectedDuration(durationOption);
    setCurrentStep(5);
  };

  const handleGroupSelection = (groupOption: any) => {
          ...
          {[1, 2, 3, 4, 5].map((step) => (
            <div
              ...
              className={`w-3 h-3 rounded-full transition-all duration-300 ${
                currentStep >= step ? 'bg-terracotta-400' : 'bg-gray-300'
          ...
          {currentStep === 2 && selectedOption && (
            ...
            >
              ...
              <div className="grid grid-cols-1 gap-4">
                {subOptions[selectedOption as keyof typeof subOptions].map((subOption, index) => (
                  <motion.button
                    key={subOption.id}
