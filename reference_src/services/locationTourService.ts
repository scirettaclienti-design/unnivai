...
interface LocationBasedRecommendations {
  city: string;
  weather: string;
...
class LocationTourService {
  ...
  ): Promise<LocationBasedRecommendations> {
    
    const weather = await aiRecommendationService.getCurrentWeather(city);
    const quickTours = this.getQuickToursForCity(city, weather.condition);
  ...
  private getQuickToursForCity(city: string, weatherCondition: string): QuickTour[] {
    const cityTours: Record<string, QuickTour[]> = {
      'Roma': [
        {
  ...
  private generatePersonalizedWelcome(userName: string, city: string, weather: any): string {
  ...
  private getWeatherMessage(condition: string): string {
  ...
  private getLocalTips(city: string): string[] {
